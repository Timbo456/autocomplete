...
public class KdTreePointST<Value> implements PointST<Value> {
   ...
   private Value get(Node x, Point2D p, boolean lr) {
    ...
    // All points in the symbol table, in level order.
     public Iterable<Point2D> points() {
        ...
        while (!q.isEmpty()) {
            Node x = q.dequeue();
            p.enqueue(x.p);
            if (x.lb != null) { q.enqueue(x.lb); }
            if (x.rt != null) { q.enqueue(x.rt); }
             //p.enqueue(x.p);
        }
        return p;
    }
    ...
    private Point2D nearest(Node x, Point2D p, Point2D nearest, 
        ...
        if (!x.p.equals(p)
            && x.p.distanceSquaredTo(p) < nearestDistance) {
            nearest = x.p;
    ...
    public Iterable<Point2D> nearest(Point2D p, int k) {
        MaxPQ<Point2D> pq1 = new MaxPQ<Point2D>(k, p.distanceToOrder());
        nearest(root, p, k, pq1, true);
        return pq1;
    }
